...
import { ChevronDown, ChevronUp, CheckCircle2, Bot, BrainCircuit, CheckSquare, Square } from 'lucide-react';

const QuestionCard = ({ data, index, isCompleted, onToggleComplete }) => {
  ...
  return (
    <div className={`rounded-xl shadow-sm border overflow-hidden transition-all duration-300 ${isCompleted ? 'bg-green-50/50 border-green-200 opacity-75' : 'bg-white border-gray-200'}`}>
      ...
      {!isMCQ && (
        ...
        >
          {isRevealed ? (
            <><span className="mr-1">Hide Solutions</span> <ChevronUp className="w-4 h-4"/></>
          ) : (
            <><span className="mr-1">Show Solutions</span> <ChevronDown className="w-4 h-4"/></>
          )}
        </button>
      )}
    </div>
