    ...
    <div className={`relative rounded-xl shadow-sm border overflow-hidden transition-all duration-300 ${getCardBackground()} ${isFlagged ? 'ring-2 ring-orange-300 ring-offset-2' : ''}`}>
       ...
       <div className="bg-slate-50/50 px-5 py-3 border-b border-gray-100 flex justify-between items-center">
         <div className="flex gap-2 text-xs font-semibold">
          <span className="px-2 py-1 bg-teal-100 text-teal-700 rounded border border-teal-200">{data.topic}</span>
          <span className="px-2 py-1 bg-slate-200 text-slate-600 rounded border border-slate-300 truncate max-w-[150px] md:max-w-none">{data.subtopic}</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2"><span className="text-xs font-mono font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded">{data.id}</span><span className="text-[10px] text-gray-400 font-mono">QUID:{data.unique_id}</span></div>
        </div>
      </div>
      ...
      <div className="p-5">
        <h3 className="text-lg text-gray-800 font-medium leading-relaxed mb-6">
          ...
          {isLocked ? (
             <span className="blur-sm select-none opacity-50">This question content is hidden because you have reached your trial limit. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</span>
          ) : (
             <span className="whitespace-pre-line [&_img]:max-w-full [&_img]:rounded-lg [&_img]:mt-2 [&_img]:border [&_img]:border-gray-200" dangerouslySetInnerHTML={createSafeMarkup(data.question)} />
          )}
        </h3>
          ...
          <div className="flex flex-col gap-3">
            {data.options.map((opt, i) => (
              ...
              >
                ...
                <span className="whitespace-pre-line" dangerouslySetInnerHTML={createSafeMarkup(opt)}/>
              </button>
            ))}
          </div>
        )}
        ...
        {!isMCQ && !isLocked && (
            <div className="mb-4">
                <div onClick={isGuest ? () => onTextChange('TRIGGER_LOGIN') : undefined}> 
                    <RichTextEditor 
                        value={localInput} 
        ...
        {isRevealed && !isLocked && (
          <div className="mt-6 flex flex-col gap-4 animate-in fade-in slide-in-from-top-4 duration-300">
             <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
               <div className="flex items-center gap-2 mb-2 text-emerald-800 font-bold text-sm uppercase tracking-wide"><CheckCircle2 className="w-4 h-4" />Goddisk Answer</div>
               <div className="text-emerald-900 leading-relaxed whitespace-pre-line [&_img]:max-w-full [&_img]:rounded-lg [&_img]:mt-2" dangerouslySetInnerHTML={createSafeMarkup(data.official_answer)}/>
             </div>
             ...
             {data.ai_answer && (
              <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4 relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-2 mb-2 text-indigo-700 font-bold text-sm uppercase tracking-wide"><Bot className="w-4 h-4" />AI Summary</div>
                  <div className="flex items-start gap-3"><span className={`shrink-0 text-[10px] font-bold px-2 py-0.5 rounded uppercase mt-0.5 border ${data.ai_answer.agreement === 'Agree' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>{data.ai_answer.agreement}</span><p className="text-indigo-900 leading-relaxed italic text-sm">"{data.ai_answer.explanation}"</p></div>
                </div>
              </div>
            )}
            ...
            <div className="border border-violet-100 rounded-lg overflow-hidden bg-white shadow-sm mt-4">
                {analysisData && (
                  <div className="p-5 bg-gradient-to-br from-white to-violet-50/30 animate-in fade-in duration-500">
                     <div className="flex items-start justify-between mb-3 pb-3 border-b border-violet-100">
                        <div className="flex items-center gap-2"><Sparkles className="w-4 h-4 text-violet-600" /><span className="font-bold text-sm text-violet-900 uppercase tracking-wide">Professor AI's Analysis ({AI_MODELS.find(m => m.id === selectedModel)?.name})</span></div>
                        {analysisCost !== null && analysisCost !== undefined && !isNaN(analysisCost) && (
                            <div className="flex flex-col items-end">
                                <div className={`flex items-center gap-1.5 px-2 py-1 rounded text-[10px] font-mono border ${analysisCost === 0 ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-violet-100 text-violet-700 border-violet-200'}`}>{analysisCost === 0 ? <><span className="font-bold">PAID</span><span>(Free)</span></> : <><span className="font-bold">COST:</span><span>${Number(analysisCost * costMultiplier).toFixed(5)}</span></>}</div>
                                {isStandardUser && <span className="text-[9px] text-gray-400 mt-0.5">Deducted from credits</span>}
                            </div>
                        )}
                     </div>
                     <div className="text-slate-700 font-serif text-sm leading-relaxed min-h-[150px]">
                       <ReactMarkdown remarkPlugins={[remarkMath]} rehypePlugins={[rehypeKatex]} components={{ h1: ({node, ...props}) => <h1 className="text-lg font-bold text-violet-900 mt-4 mb-2" {...props} />, h2: ({node, ...props}) => <h2 className="text-base font-bold text-violet-800 mt-4 mb-2 uppercase tracking-wide" {...props} />, h3: ({node, ...props}) => <h3 className="text-sm font-bold text-violet-700 mt-3 mb-1" {...props} />, p: ({node, ...props}) => <p className="mb-3" {...props} />, ul: ({node, ...props}) => <ul className="list-disc pl-5 mb-3 space-y-1" {...props} />, li: ({node, ...props}) => <li className="pl-1" {...props} />, strong: ({node, ...props}) => <strong className="font-bold text-violet-950" {...props} /> }}>{analysisData}</ReactMarkdown>
                     </div>
                  </div>
                )}
                ...
                <div className={`bg-slate-50/50 p-4 ${analysisData ? 'border-t border-violet-100' : ''}`}>
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2"><Bot className="w-4 h-4 text-violet-500" /><span className="text-xs font-bold text-gray-500 uppercase tracking-wider">{analysisData ? 'Switch Model / Compare' : 'Ask Professor AI'}</span></div>
                    </div>
                        ...
                        {AI_MODELS.map((model) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative flex flex-col items-center justify-center p-2 rounded-lg border transition-all duration-200 ${model.style} ${isThisLoading ? 'bg-violet-100 ring-2 ring-violet-300 border-transparent' : ''} ${isCurrentView ? 'ring-2 ring-violet-500 ring-offset-1 border-transparent bg-white' : 'bg-white'} ${isAnyLoading && !isThisLoading ? 'opacity-40 cursor-not-allowed grayscale' : 'cursor-pointer hover:-translate-y-0.5 hover:shadow-sm'}`}
                                >
                                    {isThisLoading ? (<Loader2 className="w-5 h-5 text-violet-600 animate-spin my-1" />) : (
                                        <>
                                            ...
                                            <span className={`text-[9px] font-mono px-1.5 py-0.5 rounded border ${badgeStyle}`}>{costText}</span>
                                        </>
                                    )}
                                </button>
                            );
                    ...
                    {loadingModel && (<div className="mt-3 text-center"><p className="text-xs text-violet-600 font-bold uppercase tracking-wider animate-pulse">Consulting {AI_MODELS.find(m => m.id === loadingModel)?.name}...</p></div>)}
                </div>
                ) : (
                    analysisData && (<div className="bg-slate-50 p-2 text-center border-t border-violet-100"><p className="text-[10px] text-gray-400 italic">AI features are currently disabled.</p></div>)
                )}

                {analysisError && (<div className="p-3 bg-red-50 flex items-center justify-between text-xs text-red-600 border-t border-red-100 animate-in slide-in-from-top-2"><div className="flex items-center gap-2"><AlertCircle className="w-4 h-4" /><span>{analysisError}</span></div><button onClick={() => {}} className="underline hover:text-red-800">Dismiss</button></div>)}
            </div>
            )}
          </div>
        )}
      ...
      {!isMCQ && !isLocked && (<button onClick={toggleReveal} className="w-full py-3 bg-gray-50 border-t border-gray-100 text-sm font-semibold text-gray-600 hover:text-teal-600 hover:bg-gray-100 transition-colors flex items-center justify-center gap-2">{isRevealed ? <><span className="mr-1">Hide Solutions</span> <ChevronUp className="w-4 h-4"/></> : <><span className="mr-1">Show Solutions</span> <ChevronDown className="w-4 h-4"/></>}</button>)}
    </div>
  );
};
