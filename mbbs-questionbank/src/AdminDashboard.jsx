...
import { Trophy, Users, BrainCircuit, X, Loader2, Target, ChevronDown, ChevronUp, BarChart3, Folder, FileText, RefreshCw, DollarSign, Coins, ArrowUpDown, ArrowUp, ArrowDown, Database, AlertCircle, Clock } from 'lucide-react';
...
const AdminDashboard = ({ onClose, questions }) => {
  ...
  const [allAiLogs, setAllAiLogs] = useState([]); 
  
  // UI State
  const [expandedUserId, setExpandedUserId] = useState(null);
  const [loadingDetails, setLoadingDetails] = useState(false); 
  const [sortConfig, setSortConfig] = useState({ key: 'lastActive', direction: 'desc' }); // Default sort by Last Active
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (statsError) {
          ...
          throw new Error("Could not load dashboard stats. Please run the SQL setup script.");
      }
      ...
      // Process User Stats
      const validStats = userStatsRaw || [];
      const processedStats = validStats.map(u => {
        ...
        return {
            ...
            lastActive: u.last_active_at, // NEW FIELD
  ...
  const sortedStats = useMemo(() => {
    ...
    if (sortConfig.key) {
      items.sort((a, b) => {
        ...
        // Date Sorting (Handle Nulls)
        if (sortConfig.key === 'lastActive') {
            if (!aVal) return 1; // Put nulls at bottom
            if (!bVal) return -1;
    ...
    return (
        <div className="fixed inset-0 z-[70] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95">
                ...
                <div className="flex-1 overflow-y-auto p-0 relative">
                    {isLoading ? <div className="absolute inset-0 flex items-center justify-center bg-white/80"><Loader2 className="animate-spin text-violet-600"/></div> : (
                        <table className="w-full text-sm text-left">
                            <thead className="text-xs text-gray-500 uppercase bg-gray-50 sticky top-0 border-b border-gray-200"><tr><th className="px-6 py-3">Time</th><th className="px-6 py-3">Question</th><th className="px-6 py-3">Model</th><th className="px-6 py-3 text-right">Cost (HKD)</th></tr></thead>
                ...
                <div className="px-6 py-3 bg-gray-50 border-t border-gray-200 text-right"><span className="text-xs font-bold text-violet-600 mr-2">TOTAL:</span><span className="text-lg font-mono font-bold text-violet-900">${user.aiCostHKD.toFixed(4)}</span></div>
            </div>
        </div>
    );
  };
  ...
  if (errorMsg) return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-white/90">
        <div className="bg-red-50 p-6 rounded-lg text-red-800 text-center max-w-md border border-red-200 shadow-xl">
            <AlertCircle className="w-10 h-10 mx-auto mb-3 text-red-500"/>
            <h2 className="text-lg font-bold mb-2">Dashboard Error</h2>
            <p className="text-sm mb-4">{errorMsg}</p>
            <button onClick={onClose} className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700">Close Dashboard</button>
        </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-slate-100 overflow-auto animate-in slide-in-from-bottom duration-300">
      ...
      <div className="bg-indigo-900 text-white sticky top-0 z-10 px-6 py-4 flex justify-between items-center shadow-lg">
        <div className="flex items-center gap-3"><div className="p-2 bg-indigo-800 rounded-lg"><Trophy className="w-6 h-6 text-yellow-400" /></div><div><h1 className="text-xl font-bold">Admin Dashboard</h1><p className="text-xs text-indigo-300">Performance & Quota Analytics</p></div></div>
