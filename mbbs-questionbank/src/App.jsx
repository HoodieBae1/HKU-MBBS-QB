...
const App = () => {
	...
	const [selectedSubtopic, setSelectedSubtopic] = useStickyState(
		"All",
	...
	const handleDownloadData = async () => {
		...
		link.href = url;
		link.download = `my_progress_data_${
	...
	const handleViewStateChange = (uniqueId, key, value) => {
		setViewState((prev) => ({
			...prev,
			[uniqueId]: { ...prev[uniqueId], [key]: value },
		}));
	};
	...
	const handleTextChange = (questionId, newText) => {
		...
		setUserProgress((prev) => {
			...
			return {
				...prev,
				[idStr]: { ...existing, question_id: idStr, user_response: newText },
			};
		});
	};
	...
	const handleAIRequest = async (questionData, modelId) => {
		...
		setAiState((prev) => ({
			...prev,
			[idStr]: { ...prev[idStr], loadingModel: modelId, error: null },
		}));
			...
			setAiState((prev) => {
				...
				return {
					...
					[idStr]: {
						...
						purchasedModels: prevPurchased.includes(modelId)
							? prevPurchased
							: [...prevPurchased, modelId],
					},
				};
			});
		} catch (err) {
	...
	const handleToggleFlag = async (questionData, currentDraftText) => {
		...
		const rawText =
			currentDraftText !== undefined
				? currentDraftText
				: currentProgress.user_response || null;
		...
		if (questionData.type === "MCQ" || questionData.type === "EMQ") {
			if (isCurrentlyCompleted && existingEntry.selected_option === mcqSelection) {
				const hasDataToKeep =
					...
					(existingEntry.user_response &&
						cleanHtmlContent(existingEntry.user_response) !== null) ||
				...
				if (hasDataToKeep) {
					...
					await supabase
						.from("user_progress")
						.delete()
						.match({ user_id: session.user.id, question_id: idString });
				}
				fetchQuotaStats(session.user.id);
				return;
			}
	...
	const filterCounts = useMemo(() => {
		...
		const baseSet = questions.filter((q) => {
			...
			return (
				...
				String(q.unique_id).toLowerCase().includes(qLower) ||
		...
		const tCounts = {};
		baseSet.forEach((q) => {
			tCounts[q.topic] = (tCounts[q.topic] || 0) + 1;
		});
		const sCounts = {};
		baseSet.forEach((q) => {
	...
	const filteredQuestions = useMemo(() => {
		...
		return result.sort((a, b) => {
			if (sortOrder === "Random") return a.randomSeed - b.randomSeed;
			if (sortOrder === "Notes") {
	...
	return (
		<div
			className={`min-h-screen bg-slate-50 text-slate-900 font-sans pb-20 relative transition-[margin] duration-300 ease-in-out ${
				modalOpen ? "md:mr-[600px]" : "mr-0"
			}`}
		>
			<VersionSupervisor />
			<ReleaseNotesModal
						...
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<div className="flex items-center gap-2 text-[10px] text-teal-200 uppercase tracking-wider">
									...
									<span className="px-1.5 py-0.5 bg-teal-800 rounded text-teal-100 opacity-80 font-mono">
										v{APP_VERSION}
									</span>
						...
						<div className="flex items-center gap-2">
							{isGuest ? (
								<button
									...
									className="bg-white text-teal-700 hover:bg-teal-50 px-4 py-2 rounded-lg font-bold text-sm shadow-sm transition-colors flex items-center gap-2"
								>
								...
								<>
									...
									<button
										...
										className="p-2 hover:bg-teal-600 rounded-full transition ml-1"
									>
						...
						<div className="flex items-center justify-between gap-4">
							<div className="flex-grow flex items-center gap-3">
								<div
									...
									className={`flex-grow flex flex-col justify-center cursor-pointer group p-2 -ml-2 rounded-lg transition-colors ${
										showProgressPanel ? "bg-indigo-50" : "hover:bg-gray-50"
									}`}
								>
									<div className="flex justify-between text-xs font-bold text-gray-500 mb-1">
										...
										<span className="text-teal-600">
											{completedCount} / {totalQuestionsCount} ({progressPercentage}
											%)
										</span>
									</div>
							...
							<button
								...
								className={`p-2 rounded-lg border transition-all duration-200 flex items-center gap-2 text-sm font-semibold ${
									filtersOpen
						...
						<div className="relative">
								...
								<div className="min-h-0 flex flex-col gap-3">
									...
									<div className="grid grid-cols-1 md:grid-cols-4 gap-3">
										<div className="relative">
											...
											>
												<option value="All">
													All Topics ({filterCounts.totalMatchingSearch})
												</option>
												{topicsList.map((t) => (
													<option key={t} value={t}>
														{t} ({filterCounts.tCounts[t]})
													</option>
												))}
											</select>
										...
										<div className="relative" ref={typeDropdownRef}>
											<button
												...
												className="w-full pl-3 pr-8 py-2 border border-gray-300 rounded-lg text-sm bg-white text-left flex items-center justify-between truncate"
											>
											...
											{showTypeDropdown && (
												<div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-xl z-50 p-2 space-y-1">
													{["MCQ", "EMQ", "SAQ"].map((type) => (
														<div
															key={type}
															onClick={() => handleTypeToggle(type)}
															className="flex items-center gap-2 px-2 py-1.5 hover:bg-gray-50 rounded cursor-pointer select-none"
														>
															<div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${selectedTypes.includes(type) ? 'bg-teal-600 border-teal-600' : 'border-gray-300 bg-white'}`}>
																{selectedTypes.includes(type) && <CheckSquare className="w-3 h-3 text-white" />}
															</div>
										...
										<div className="relative">
											<select
												...
												className="w-full pl-3 py-2 border border-gray-300 rounded-lg text-sm appearance-none bg-white"
											>
							...
							<div
								className={`grid transition-all duration-500 ease-in-out ${
									showUserStats
										? "grid-rows-[1fr] opacity-100 border-t border-gray-100 shadow-lg"
							...
							return (
								<div className="pb-6">
									<QuestionCard
										...
										onToggleReveal={(val) =>
											handleViewStateChange(idStr, "isRevealed", val)
										}
										...
										onReviewNotes={(draft, viewMode) =>
											handleReviewNotes(q, draft, viewMode)
										}
