...
import { Coins, Info, Loader2, X, History, FileText, Calendar, Sparkles } from 'lucide-react';
...
const AIUsageDisplay = ({ session }) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { 
        ...
        // Optimistically add new log to top
  ...
  const fetchData = async () => {
    // Fetch detailed logs ordered by newest first
        ...
        <div className={`absolute top-full right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-gray-200 p-4 transition-all duration-200 origin-top-right z-50 ${showTooltip && !isModalOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
          <div className="flex items-start gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                Click to view detailed history. You pay per click based on the model complexity. Cached answers are <strong>Free</strong>.
              </p>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-lg p-3 space-y-2 border border-gray-100">
            ...
            <div className="pt-2 mt-2 border-t border-gray-200 flex justify-between text-xs font-bold text-indigo-700">
              ...
              <span>${stats.totalCost.toFixed(5)}</span>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-gray-800">AI Usage History</h2>
                            <p className="text-xs text-gray-500">Breakdown of all your interactions with the AI Professor.</p>
                        </div>
                    </div>
                    <button 
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 bg-white border-b border-gray-100">
                    <div className="p-4 rounded-lg bg-indigo-50 border border-indigo-100">
                        <p className="text-xs font-bold text-indigo-400 uppercase tracking-wider mb-1">Total Spent</p>
                        <p className="text-2xl font-mono font-bold text-indigo-900">${stats.totalCost.toFixed(5)}</p>
                    </div>
                    <div className="p-4 rounded-lg bg-slate-50 border border-slate-100">
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Interactions</p>
                        <p className="text-2xl font-mono font-bold text-slate-700">{logs.length}</p>
                    </div>
                    <div className="p-4 rounded-lg bg-emerald-50 border border-emerald-100">
                        <p className="text-xs font-bold text-emerald-400 uppercase tracking-wider mb-1">Free (Cached) Hits</p>
                        <p className="text-2xl font-mono font-bold text-emerald-700">
                            {logs.filter(l => l.cost === 0).length}
                        </p>
                    </div>
                </div>
                                ...
                                logs.map((log) => (
                                    <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-3 text-right">
                                            {log.cost === 0 ? (
                                                <span className="px-2 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-bold border border-emerald-200">
                                                    Free
                                                </span>
                                            ) : (
